...
from 功能函数.k_best import k_best_feature_selection
from 功能函数.sample_funtion import softmax_sample
...
# 定义DQNAgent
class DQNAgent:
    def __init__(self, state_size, action_size, epsilon_num=3000, memory_size=200, batch_size=32, gamma=0.95, epsilon=1,
                 epsilon_decay=0.1,
                 epsilon_min=0.1,learning_rate=0.01):
...
if __name__ == "__main__":
    ...
    w_file_name = 'k_best_0.3+softmax.txt'
    ...
    Y = dataset.iloc[:, (c - 1)]
    K_best_feature = k_best_feature_selection(X, Y, 38)
    ...
    best_accracy = 0
    num_inter_rate = 0.3
    for es in range(int(esposide_n * num_inter_rate)):
        tem_act = np.zeros(n_feature)
        for i in range(len(dqn_list)):
            tem_act[i] = dqn_list[i].select_action(torch.tensor(s, dtype=torch.float32))
        selected_indices = np.where(K_best_feature)[0]
        half_size = len(selected_indices) // 2
        random_half_indices = np.random.choice(selected_indices, size=half_size, replace=False)
        random_half = np.zeros_like(K_best_feature, dtype=bool)
        random_half[random_half_indices] = 1
        action_list = np.logical_or(tem_act, random_half)
        X_selected = X_train.iloc[:, action_list]
        ...
        if es % num == 0:
            tem = ave / num
            ave = 0
            with open(w_file_path,
                      'a') as file:
                file.write(f'ES: {es}    acc: {tem}, \n')
    for es in range(int(esposide_n * num_inter_rate), esposide_n):
        for i in range(len(dqn_list)):
